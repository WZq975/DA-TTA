...
from methods.augmentations.transforms_cotta import get_tta_transforms
...
class RoTTA(TTAMethod):
    ...
    def forward_and_adapt(self, x):
        ...
        with torch.no_grad():
            ...
            # ZqW
            if cfg.CORRUPTION.DATASET == "imagenet_d":
                mapping_vector = list(IMAGENET_D_MAPPING.values())
                pseudo_label = torch.tensor([mapping_vector[pred] for pred in pseudo_label], device='cuda')
...
class CSTU:
    ...
    def remove_instance(self, cls, score):
        # print(len(self.data), cls, self.num_class)
