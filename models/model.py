...
from datasets.imagenet_subsets import IMAGENET_A_MASK, IMAGENET_R_MASK
...
class ImageNetXWrapper(torch.nn.Module):
    ...
    def forward(self, x):
        if cfg.MODEL.ADAPTATION == 'hp':
            logits, stat, rep = self.model(self.normalize(x))
            return self.masking_layer(logits), stat, rep
        else:
...
def split_up_model(model, arch_name, dataset_name):
    ...
    if hasattr(model, "model") and hasattr(model.model, "pretrained_cfg") and hasattr(model.model, model.model.pretrained_cfg["classifier"]):
        ...
        classifier = model.fc
    elif "resnet" in arch_name or "resnext" in arch_name or "wide_resnet" in arch_name or arch_name in {"Standard_R50", "Hendrycks2020AugMix", "Hendrycks2020Many", "Geirhos2018_SIN", 'res2net101_26w_4s'}:
